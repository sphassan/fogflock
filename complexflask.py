...
def hello_world():
    try:
        ...
        for i in range(loops):
            ...
            with connection.cursor() as cursor:
                sql = "SELECT followed_id, SUM(relevancy) FROM fogflock.relevancy GROUP BY followed_id ORDER BY SUM(relevancy) DESC"
                cursor.execute(sql)
            ...
            for result in top_results:
                ...
                if(relevancy_percent > relevance_threshold**(i+1)):
        ...
        with connection.cursor() as cursor:
            ...
            return(json.dumps(outputstufftotal))
        connection.close()
    except:
        return 'oops'
